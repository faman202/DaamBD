...
export const PriceConverter: React.FC<PriceConverterProps> = ({
  ...
  const [mode, setMode] = useState<'weight-to-price' | 'price-to-weight'>('weight-to-price');
  const [weightValue, setWeightValue] = useState<number>(500); // in grams by default
  const [budgetValue, setBudgetValue] = useState<number>(100);  // in BDT
  const [unitType, setUnitType] = useState<'g' | 'kg' | 'palla' | 'hali'>('g');

  if (!isOpen || !item) return null;

  const currentPriceObj = priceType === 'retail' ? item.retail : item.wholesale;
  const avgPricePerBaseUnit = currentPriceObj.avgPrice; // per kg or per hali

  // Presets
  const presets = [
    { labelBn: '১০০ গ্রাম', labelEn: '100 g', weightInGrams: 100 },
    { labelBn: '২৫০ গ্রাম', labelEn: '250 g', weightInGrams: 250 },
    { labelBn: '৫০০ গ্রাম (আধ কেজি)', labelEn: '500 g', weightInGrams: 500 },
    { labelBn: '১ কেজি', labelEn: '1 kg', weightInGrams: 1000 },
    { labelBn: '২ কেজি', labelEn: '2 kg', weightInGrams: 2000 },
    { labelBn: '৫ কেজি (১ পাল্লা)', labelEn: '5 kg (1 Palla)', weightInGrams: 5000 },
  ];

  // Calculation Logic
  // If item is eggs (sold in hali), calculations adjust to count
  const isPerHali = currentPriceObj.unitEn.toLowerCase().includes('hali') || currentPriceObj.unitBn.includes('হালি');
  ...
  if (mode === 'weight-to-price') {
    if (isPerHali) {
      // weightValue interpreted as number of eggs / halis
      const eggsCount = weightValue;
      calculatedResultCost = Math.round((eggsCount / 4) * avgPricePerBaseUnit);
    } else {
      // Weight in grams converted to kg
      const kg = weightValue / 1000.0;
      calculatedResultCost = Math.round(kg * avgPricePerBaseUnit);
    }
  } else {
    // Budget to Weight
    if (isPerHali) {
      const halis = budgetValue / avgPricePerBaseUnit;
      const totalEggs = Math.floor(halis * 4);
      calculatedResultAmount = lang === 'bn' 
        ? `${toBanglaNumber(totalEggs)} টি ডিম (${toBanglaNumber(Math.round(halis * 10) / 10)} হালি)`
        : `${totalEggs} eggs (${Math.round(halis * 10) / 10} hali)`;
    } else {
      const kgObtained = budgetValue / avgPricePerBaseUnit;
      if (kgObtained < 1.0) {
        const grams = Math.round(kgObtained * 1000);
        calculatedResultAmount = lang === 'bn' ? `${toBanglaNumber(grams)} গ্রাম` : `${grams} grams`;
      } else {
        const roundedKg = Math.round(kgObtained * 100) / 100;
        calculatedResultAmount = lang === 'bn' ? `${toBanglaNumber(roundedKg)} কেজি` : `${roundedKg} kg`;
      }
    }
  }
      ...
      <div className="bg-white rounded-3xl max-w-lg w-full overflow-hidden shadow-2xl border border-surface-border animate-in zoom-in-95 duration-200">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-xs text-emerald-100 font-medium">
                {lang === 'bn' ? item.nameBn : item.nameEn} (গড় দর: {formatPrice(avgPricePerBaseUnit, lang)} / {lang === 'bn' ? currentPriceObj.unitBn : currentPriceObj.unitEn})
              </p>
            </div>
          </div>
        ...
        <div className="p-5 sm:p-6 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-2 bg-surface-bg p-1.5 rounded-xl border border-surface-border">
            ...
            <button
              onClick={() => setMode('price-to-weight')}
              className={`py-2 px-3 rounded-lg text-xs font-bold transition-all flex items-center justify-center space-x-1.5 ${
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="grid grid-cols-3 sm:grid-cols-3 gap-2">
                  {presets.map((p) => {
                    const isSelected = weightValue === p.weightInGrams;
                    return (
                      <button
                        key={p.weightInGrams}
                        onClick={() => handlePresetSelect(p.weightInGrams)}
              ...
              {/* Custom Grams / Unit Slider or Input */}
              <div className="bg-surface-bg p-4 rounded-2xl border border-surface-border space-y-2">
                <div className="flex items-center justify-between text-xs font-bold text-content-main">
                  <span>{lang === 'bn' ? 'কাস্টম ওজন নির্বাচন (গ্রাম)' : 'Custom Weight (Grams)'}</span>
                  <span className="text-brand-700 text-sm">
                    {lang === 'bn' ? `${toBanglaNumber(weightValue)} গ্রাম` : `${weightValue} g`}
                  </span>
                </div>
                <input
                  type="range"
              ...
              <div className="bg-gradient-to-br from-emerald-50 to-green-100/60 p-4 rounded-2xl border border-emerald-300 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-emerald-700 font-medium mt-0.5">
                    ({lang === 'bn' ? `${toBanglaNumber(weightValue)} গ্রাম এর জন্য` : `For ${weightValue}g`})
                  </p>
                </div>
                <div className="text-right">
                  <span className="text-2xl sm:text-3xl font-black text-brand-900">
                    {formatPrice(calculatedResultCost, lang)}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-gradient-to-br from-emerald-50 to-green-100/60 p-4 rounded-2xl border border-emerald-300 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-emerald-700 font-medium mt-0.5">
                    ({formatPrice(budgetValue, lang)} {lang === 'bn' ? 'টাকায়' : 'budget'})
                  </p>
                </div>
                <div className="text-right">
                  <span className="text-2xl sm:text-3xl font-black text-brand-900">
